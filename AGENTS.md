...
- **Monthly review cron:** Vercel Cron hits `/api/cron/monthly-review` on the 1st of each month at 15:00 UTC (`vercel.json`). It emails all members for the previous calendar month via Resend. Requires Production env vars `CRON_SECRET` and `SUPABASE_SERVICE_ROLE_KEY` (plus `RESEND_API_KEY`). Dedupes by checking `email_tracking` for an existing `monthly-swim-review` row with the same `metadata.month`. Manual sends remain available under Admin → Emails → Monthly review.
