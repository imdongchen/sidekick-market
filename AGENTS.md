...
- **Monthly review cron:** Vercel Cron hits `/api/cron/monthly-review` on the 1st of each month at 15:00 UTC (`vercel.json`). It emails all members for the previous calendar month via Resend. Requires Production env vars `CRON_SECRET` and `SUPABASE_SERVICE_ROLE_KEY` (plus `RESEND_API_KEY`). Dedupes per recipient via `email_tracking` (`metadata.campaign` + `metadata.month`); a retry only emails people who do not already have a sent/scheduled row. After each run (including partial failure or nothing-to-send), emails a summary to `admin@sidekickswim.com` (override with `MONTHLY_REVIEW_ADMIN_EMAIL`). Manual sends remain available under Admin → Emails → Monthly review.
